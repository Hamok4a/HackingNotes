...
# Without the msg connect without the msg : 
```javascript
Linkedin = {
  config: {
    scrollDelay: 3000,
  ...
  clickSendWithoutNote: function (data, config) {
    var sendWithoutNoteButton = document.querySelector(
      'button[aria-label="Send without a note"]'
    );
    if (sendWithoutNoteButton) {
      sendWithoutNoteButton.click();
      console.debug("DEBUG: Send without a note button clicked");
    } else {
      console.debug(
        "DEBUG: send without a note button not found, clicking send button directly in " +
          config.actionDelay
      );
      setTimeout(() => this.clickSendDirectly(data, config), config.actionDelay);
    }
    setTimeout(() => this.clickClose(data, config), config.actionDelay);
  },
  clickSendDirectly: function (data, config) {
    var buttons = document.querySelectorAll("button");
    var sendButton = Array.prototype.filter.call(buttons, function (el) {
      return el.textContent.trim() === "Send now" || el.textContent.trim() === "Send";
    });
    // Click the first send button
